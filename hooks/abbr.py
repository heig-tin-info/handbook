...
class AbbrWikiExtension(AbbrExtension):
    def extendMarkdown(self, md):
        ...
        md.parser.blockprocessors.register(AbbrWikiBlockprocessor(md.parser, self.abbrs), 'abbrwiki', 17)
                ...
                if self.abbrs[m.group(0)]:
                    if el.tag == 'a':
                        if el.text == m.group(0):
                            ...
                            log.warning("Can't wrap abbreviation in existing link: %s, ignoring!", m.group(0))
                    else:
...
class AbbrWikiBlockprocessor(AbbrBlockprocessor):
    RE = re.compile(r'^[*]\[(?P<abbr>[^\\]*?)\][ ]?:[ ]*\n?[ ]*(?P<title>https?://[a-z]{2,}\.wikipedia\.org/.*)$', re.MULTILINE)
