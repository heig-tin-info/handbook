...
Sub export(sourcePath, destPath, widthInPixels, heightInPixels)
    ...
    visioApplication.Documents.OpenEx sourcePath, visOpenRO + visOpenMinimized + visOpenHidden + visOpenMacrosDisabled + visOpenNoWorkspace
    visioApplication.ActiveDocument.Pages.Item(1).Export destPath
...
Dim sourcePath : sourcePath = currentDirectory & "\" & Replace(WScript.Arguments(0), "/", "\")
Dim destPath : destPath = currentDirectory & "\" & Replace(WScript.Arguments(1), "/", "\")
...
Wscript.Echo destPath
export sourcePath, destPath, 3557, 4114
